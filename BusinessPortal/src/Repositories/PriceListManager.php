...
class PriceListManager
{
    ...
    public function __construct($pdo)
    {
        ...
        // Absolute path to BusinessPortal/auth/uploads/pricelist/ — avoids DOCUMENT_ROOT pitfalls
        $this->uploadsDir = realpath(__DIR__ . '/../..') . '/auth/uploads/pricelist/';
    ...
    /**
     * Get a download URL for a price list.
     *
     * Routes through the secure `auth/download-pricelist.php` endpoint
     * which checks authentication + per-account assignment before streaming
     * the file. The raw upload folder is blocked from direct web access.
     *
     * @param int|string|array $idOrRow  Either an integer ID, or the price-list row.
     * @param bool             $preview  When true, opens inline (PDF/img preview).
     */
    public function getFileUrl($idOrRow, bool $preview = false)
    {
        $id = is_array($idOrRow) ? (int)($idOrRow['id'] ?? 0) : (int)$idOrRow;
        $qs = 'id=' . $id . ($preview ? '&preview=1' : '');
        return bp_url('/auth/download-pricelist.php?' . $qs);
    }
    ...
    {
        return file_exists($this->uploadsDir . $fileName);
    }
